...
class S3Connector(object):
	...
	def _create_clients(self, aws_access_key_id=None, aws_secret_access_key=None, aws_region=None):
		...
		if None not in [self._aws_access_key_id, self._aws_secret_access_key, self._aws_region]:
	...
	def upload_dataframe_as_csv(self, dataframe=None, bucket_name=None, object_name=None):
		...
		try:
			...
			csv_io = io.StringIO()
			dataframe.to_csv(csv_io, sep=self.csv_sep, encoding='utf-8-sig',
							 header=True, index=False, na_rep=self.csv_null_identifier)
			csv_contents = csv_io.getvalue()
			csv_contents = re.sub(r'NaT', self.csv_null_identifier, self.csv_contents)
			csv_io.seek(0)
			...
			if not self._exists_bucket(bucket_name):
				self._create_bucket(bucket_name)
			self._s3_resource.Object(bucket_name, object_name).put(Body=csv_io.getvalue())
			csv_io.close()
